...
// CHIP-8 32비트 확장: 16KB 메모리를 사용합니다.
constexpr unsigned int MEMORY_SIZE = 16384;

// CHIP-8 32비트 확장: 32개의 32비트 레지스터(R0~R31)를 사용합니다.
constexpr unsigned int NUM_REGISTERS = 32;

// CHIP-8 32비트 확장: 최대 64단계의 서브루틴 호출 스택을 가집니다.
...
public:
    ...
    void set_pc(uint32_t value) { pc = value; }  // ✅ 32비트 파라미터로 수정

    // R 레지스터 접근 (32비트로 확장)
    uint32_t get_V(int index) const { return V.at(index); }
    void set_V(int index, uint32_t value) { V.at(index) = value; }
    ...
    uint32_t& stack_at(uint8_t index); // 32비트 참조 리턴

private:
    std::array<uint8_t, MEMORY_SIZE> memory;     // 16KB 메모리
    std::array<uint32_t, NUM_REGISTERS> V;       // 범용 레지스터 R0~R31 (32비트)
    uint32_t I;                                  // 인덱스 레지스터 (32비트 주소)
    uint32_t pc;                                 // 프로그램 카운터 (32비트)

    std::array<uint32_t, STACK_SIZE> stack;      // 스택 (32비트 주소 저장)
    uint8_t sp;                                  // 스택 포인터

    uint32_t opcode;                             // 현재 실행 중인 명령어 (4바이트)

    // 내부 함수: opcode 처리기 (32비트)
