...
namespace OpcodeTable {
    ...
    // 16개의 주요 명령 그룹(상위 8비트로 구분)을 처리하기 위한 함수 테이블
    std::array<OpcodeTable::OpcodeHandler, 16> primary_table;

    constexpr int FLAG_REG = 31; // 가장 마지막 레지스터
    ...
    void OP_3XNN(Chip8& chip8, uint32_t opcode) {
        ...
        uint8_t nn = opcode & 0x0000FFFF;
        chip8.set_pc(chip8.get_pc() + (chip8.get_R(x) == nn ? 8 : 4));
    }
    ...
    void OP_5XY0(Chip8& chip8, uint32_t opcode) {
        ...
        chip8.set_pc(chip8.get_pc() + ((opcode & 0x000000FF) == 0 && chip8.get_R(x) == chip8.get_R(y) ? 8 : 4));
    }

    /// @brief Vx에 NN 저장 (6XNN)
    void OP_6XNN(Chip8& chip8, uint32_t opcode) {
        uint8_t x = (opcode & 0x00FF0000) >> 16;
        chip8.set_R(x, opcode & 0x0000FFFF);
        chip8.set_pc(chip8.get_pc() + 4);
    }
    ...
    void OP_7XNN(Chip8& chip8, uint32_t opcode) {
        uint8_t x = (opcode & 0x00FF0000) >> 16;
        chip8.set_R(x, chip8.get_R(x) + (opcode & 0x0000FFFF));
        chip8.set_pc(chip8.get_pc() + 4);
    }
    ...
    void OP_8XYN(Chip8& chip8, uint32_t opcode) {
        uint8_t x = (opcode & 0x00FF0000) >> 16;
        uint8_t y = (opcode & 0x0000FF00) >> 8;
        uint8_t n = opcode & 0x000000FF;
        uint8_t vx = chip8.get_R(x) & 0xFF;
        uint8_t vy = chip8.get_R(y) & 0xFF;

        switch (n) {
            case 0x0: chip8.set_R(x, vy); break;
            case 0x1: chip8.set_R(x, vx | vy); break;
            case 0x2: chip8.set_R(x, vx & vy); break;
            case 0x3: chip8.set_R(x, vx ^ vy); break;
            case 0x4: {
                uint32_t sum = vx + vy;
                chip8.set_R(FLAG_REG, sum > 0xFF);  // carry flag
                chip8.set_R(x, sum & 0xFF);
                break;
            }
            case 0x5:
                chip8.set_R(FLAG_REG, vx > vy);  // borrow flag
                chip8.set_R(x, vx - vy);
                break;
            case 0x6:
                chip8.set_R(FLAG_REG, vx & 0x1);  // LSB 저장
                chip8.set_R(x, vx >> 1);
                break;
            case 0x7:
                chip8.set_R(FLAG_REG, vy > vx);  // borrow flag
                chip8.set_R(x, vy - vx);
                break;
            case 0xE:
                chip8.set_R(FLAG_REG, (vx & 0x80) >> 7);  // MSB 저장
                chip8.set_R(x, vx << 1);
                break;
        }
        chip8.set_pc(chip8.get_pc() + 4);
    ...
    void OP_BNNN(Chip8& chip8, uint32_t opcode) {
        chip8.set_pc((opcode & 0x00FFFFFF) + chip8.get_R(0));
    }

    /// @brief Vx에 rand() & NN 저장 (CXNN)
    void OP_CXNN(Chip8& chip8, uint32_t opcode) {
        uint8_t x = (opcode & 0x00FF0000) >> 16;
        chip8.set_R(x, (rand() % 256) & (opcode & 0x0000FFFF));
        chip8.set_pc(chip8.get_pc() + 4);
    }

    /// @brief 스프라이트 그리기 (DXYN)
    void OP_DXYN(Chip8& chip8, uint32_t opcode) {
        uint8_t x = chip8.get_R((opcode & 0x00FF0000) >> 16);
        uint8_t y = chip8.get_R((opcode & 0x0000FF00) >> 8);
        uint8_t height = opcode & 0x000000FF;
        chip8.set_R(0xFF, 0);  // 충돌 감지 플래그
        ...
        for (int row = 0; row < height; ++row) {
            ...
            for (int col = 0; col < 8; ++col) {
                if (sprite & (0x80 >> col)) {
                    ...
                    if (chip8.get_video(index)) chip8.set_R(0xFF, 1);
    ...
    void OP_EX(Chip8& chip8, uint32_t opcode) {
        uint8_t x = (opcode & 0x00FF0000) >> 16;
        uint8_t key = chip8.get_R(x);
        if ((opcode & 0x0000FFFF) == 0x090E)
            chip8.set_pc(chip8.get_pc() + (chip8.get_key(key) ? 8 : 4));
        else if ((opcode & 0x0000FFFF) == 0x0A01)
            chip8.set_pc(chip8.get_pc() + (!chip8.get_key(key) ? 8 : 4));
        else
            chip8.set_pc(chip8.get_pc() + 4);
    }
    ...
    void OP_FX(Chip8& chip8, uint32_t opcode) {
        ...
        uint8_t nn = opcode & 0x0000FFFF;
        switch (nn) {
            case 0x07: chip8.set_R(x, chip8.get_delay_timer()); break;
            case 0x0A: {
                for (int i = 0; i < 16; ++i) {
                    if (chip8.get_key(i)) {
                        chip8.set_R(x, i);
            ...
            case 0x15: chip8.set_delay_timer(chip8.get_R(x)); break;
            case 0x18: chip8.set_sound_timer(chip8.get_R(x)); break;
            case 0x1E: chip8.set_I(chip8.get_I() + chip8.get_R(x)); break;
            case 0x29: chip8.set_I(chip8.get_R(x) * 5); break;  // 폰트 주소
            case 0x33: {  // BCD 변환
                uint8_t vx = chip8.get_R(x);
            ...
            case 0x55:
                for (int i = 0; i <= x; ++i)
                    chip8.set_memory(chip8.get_I() + i, chip8.get_R(i));
                break;
            case 0x65:
                for (int i = 0; i <= x; ++i)
                    chip8.set_R(i, chip8.get_memory(chip8.get_I() + i));
                break;
        }
        chip8.set_pc(chip8.get_pc() + 4);
    }
    ...
    void Initialize() {
        ...
        primary_table[0x0] = [](Chip8& chip8, uint32_t opcode) {
            switch (opcode & 0x0000FFFF) {
                case 0x0E00: OP_00E0(chip8, opcode); break;
                case 0x0E0E: OP_00EE(chip8, opcode); break;
                default:
    ...
    void Execute(Chip8& chip8, uint32_t opcode) {
        // 어차피 추출하는 opcode 값은 8bit니까 index는 그대로 uint8_t로 둬도 됨 (change)
