...
void print_stack_frame(const Chip8_32& chip8_32) {
    ...
    std::cout << "Stack Used: " << std::dec << used << " bytes" << std::endl;
    std::cout << "Stack Free: " << std::dec << free << " bytes" << std::endl;
...
void dump_stack_memory(const Chip8_32& chip8_32, uint32_t start_addr, uint32_t end_addr) {
    ...
    // 4바이트씩 출력 (32비트 워드 단위)
    constexpr uint32_t MEMORY_SIZE_32 = 65536; // chip8_32.hpp에서 가져온 값
